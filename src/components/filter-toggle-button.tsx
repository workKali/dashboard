  ...
  return (
    <Button
      ...
      sx={{
        display: 'flex',
        alignItems: 'center',
        gap: (theme as any)?.custom?.spacing?.xs || '5px',
        ...
        borderRadius: `${(theme as any)?.custom?.borderRadius?.button || 8}px`,
        padding: '4px 12px',
        cursor: 'pointer',
        color: areFiltersActive ? 'rgb(22, 163, 74)' : 'rgb(58, 82, 68)',
        fontSize: 'var(--font-size-sm)',
        fontWeight: (theme as any)?.custom?.typography?.buttonSmall?.fontWeight || 600,
        transition: '0.15s',
        whiteSpace: 'nowrap',
        textTransform: (theme as any)?.custom?.typography?.buttonSmall?.textTransform || 'none',
        '&:hover': {
